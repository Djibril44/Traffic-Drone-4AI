...
# Version avec YOLO26 - Derniere generation pour une detection optimale
# Code complet pret a l'emploi
# ======================================================================
...
class VehicleDetector:
    """Detecteur utilisant YOLO26 - derniere generation ultra-rapide"""
    ...
    def _load_model(self):
        """Charge le modele YOLO26 avec fallback"""
        try:
            ...
            # Essayer differentes versions de YOLO26
            models_to_try = [
                'yolo26n.pt',    # Nano - plus rapide
                'yolo26s.pt',    # Small - equilibre
                'yolov8n.pt',    # Fallback
            ]
            ...
            if not self.use_yolo:
                st.warning("Aucun modele YOLO disponible. Mode simulation actif.")
    ...
    def detect(self, frame: np.ndarray, enabled_classes: Set[int]) -> List[Detection]:
        """Detection avec YOLO26 optimise"""
        ...
        try:
            ...
            # YOLO26 est plus performant sur les petits objets
            results = self.model(
                frame, 
            ...
            if results and results[0].boxes is not None:
                ...
                for box, conf, cls_id in zip(boxes, confs, classes):
                    ...
                    # Seuil plus bas pour capturer les petits vehicules
                    if box_w * box_h < 100 or box_w < 8 or box_h < 8:
                        continue
    ...
    def _remove_duplicates(self, detections: List[Detection], iou_threshold: float = 0.4) -> List[Detection]:
        """Supprime les detections en double"""
        if len(detections) <= 1:
            return detections
...
Les donnees presentees sont basees sur l'analyse video par YOLO26, la derniere generation
d'intelligence artificielle pour la vision par ordinateur.
...
class PDFReport(FPDF):
    ...
    def add_metadata_section(self):
        ...
        model_used = metadata.get('model_used', 'YOLO26')
        info = f"""
...
Ce rapport a ete genere automatiquement par la plateforme de detection de vehicules.
Les donnees presentees sont basees sur l'analyse video par YOLO26, la derniere generation
d'intelligence artificielle offrant une detection jusqu'a 43% plus rapide et plus precise.
...
class VehicleCounter:
    ...
    def _draw_count_panel(self, frame: np.ndarray):
        h, w = frame.shape[:2]
        panel_w = 280
        panel_h = 140 + len(self.vehicle_counts) * 22
        x, y = w - panel_w - 10, 10
        overlay = frame.copy()
        ...
        cv2.putText(frame, f"Modele: {self.metadata.get('model_used', 'YOLO26')}", (x + 10, y + 90), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (100, 200, 100), 1)
        y_offset = 115
...
with st.sidebar:
    ...
    st.caption("Derniere generation IA - Jusqu'a 43% plus rapide")
    ...
    st.info("YOLO26: Detection NMS-free, 43% plus rapide, meilleure detection petits objets")
    ...
    config.confidence_threshold = st.slider("Seuil confiance", 0.05, 0.5, config.confidence_threshold, 0.05, help="Plus bas = plus de detections")
...
with col_model3:
    st.metric("Petits objets", "Detection amelioree", delta="STAL")
...
with tab1:
    ...
    with col_video:
        ...
        with col1:
            if st.button("Démarrer", disabled=st.session_state.processing, use_container_width=True):
        ...
        if st.session_state.video_path and st.session_state.processing and not st.session_state.paused:
            if st.session_state.video_capture is None:
                ...
                counter.set_metadata(metadata)
                st.info(f"Video: {original_fps:.0f} FPS | {total_frames} frames | Duree: {metadata['video_duration']:.1f}s")
            ...
            while cap.isOpened() and st.session_state.processing and not st.session_state.paused:
                ...
                video_ph.image(
                    ...
                    caption=f"Frame {counter.frame_count}/{metadata.get('total_frames_video', 0)} | {counter.fps:.1f} FPS | Progression: {progress:.1f}% | Total: {counter.total_unique}"
                )
...
with tab2:
    ...
    if counter.total_unique > 0:
        ...
        with col3:
            ...
            st.metric("Calibration", f"{config.calibration_factor:.4f} m/px")
        st.markdown("---")
...
with tab5:
    ...
    if counter.total_unique > 0:
        ...
        st.markdown("---")
        st.info("Les exports incluent les metadonnees, les vitesses et les distances")
    else:
...
with tab6:
    ...
    if counter.total_unique > 0:
        ...
        with col_info2:
            ...
            st.metric("Distances mesurees", len(counter.distance_records))
